...
if strcmpi(source.String,'Add')
    useFeat = gui.features.menu.Value;
    useCh = gui.features.channels.Value;
else
    useFeat = 1:size(gui.features.menu.String,1);
    useCh   = gui.features.channels.Value*ones(1,size(gui.features.menu.String,1));
end

for i = 1:length(useFeat)
    feat.featNum      = useFeat(i);
    feat.ch           = useCh(i);
    ...
    feat.label        = text(feat.axes,-gui.features.win*0.975,...
                             mean(reshape(gui.data.tracking.features(:,:,feat.featNum),1,[])),...
