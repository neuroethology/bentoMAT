...
exptGui.CaMulti   = uicontrol('parent',f,fillin{:},'position',[.035 .25 .2 .03],'Tag','concatCa',...
                'style','checkbox','string','Multiple trials per Ca file','callback',@toggleVis);
exptGui.CaFRtxt   = uicontrol('parent',f,fillin{:},'position',[.035 .19 .14 .045],'Tag','concatCa',...
                'style','text','string','Ca framerate (Hz):','horizontalalignment','left');
exptGui.CaFR      = uicontrol('parent',f,fillin{:},'position',[.18 .19 .06 .05],'Tag','concatCa',...
                'style','edit','string','30');
exptGui.annoFR    = uicontrol('parent',f,fillin{:},'position',[.46 .19 .06 .05],'Tag','concatCa',...
                'style','edit','string','30');
exptGui.CaFRtog   = uicontrol('parent',f,fillin{:},'position',[.035 .15 .3 .03],'Tag','FRCa',...
                'style','checkbox','string','Variable Ca framerate','callback',@toggleVis);
exptGui.offset    = uicontrol('parent',f,fillin{:},'position',[.035 .1 .35 .03],'Tag','offset',...
                'style','checkbox','string','Time offset between Ca + annot start','callback',@toggleVis);
            
exptGui.annoMulti = uicontrol('parent',f,fillin{:},'position',[.3 .25 .25 .03],'Tag','concatAnnot',...
                'style','checkbox','string','Multiple trials per annotation file','callback',@toggleVis);
exptGui.annoFRtxt = uicontrol('parent',f,fillin{:},'position',[.3 .19 .16 .045],'Tag','concatCa',...
                'style','text','string','Anno framerate (Hz):','horizontalalignment','left');
exptGui.annoFRtog = uicontrol('parent',f,fillin{:},'position',[.3 .15 .35 .03],'Tag','FRAnnot',...
                'style','checkbox','string','Variable annot. framerate','callback',@toggleVis);
            
exptGui.incTracking = uicontrol('parent',f,fillin{:},'position',[.575 .25 .35 .03],'Tag','Tracking',...
                'style','checkbox','string','Link tracking data','value',1,'callback',@toggleVis);
exptGui.incMovies = uicontrol('parent',f,fillin{:},'position',[.575 .2 .35 .03],'Tag','BhvMovies',...
                'style','checkbox','string','Link behavior movies','value',1,'callback',@toggleVis);
exptGui.incAudio  = uicontrol('parent',f,fillin{:},'position',[.575 .15 .35 .03],'Tag','Audio',...
                'style','checkbox','string','Link audio data','callback',@toggleVis);
exptGui.inctSNE   = uicontrol('parent',f,fillin{:},'position',[.575 .1 .35 .03],'Tag','tSNE',...
                'style','checkbox','string','Link scatter data','callback',@toggleVis);


p = get(f,'position');
exptGui.bAdd      = uicontrol('parent',f,fillin{:},'position',[.025 .3 .03 .04],...
                'style','pushbutton','string','+','callback',{@addRow,0});
exptGui.bDelete   = uicontrol('parent',f,fillin{:},'position',[.06 .3 .03 .04],...
                'style','pushbutton','string',char(8211),'callback',@removeRow);
exptGui.bCopy     = uicontrol('parent',f,fillin{:},'position',[.095 .3 .03 .04],...
                'style','pushbutton','string',char(8631),'callback',{@addRow,1});
