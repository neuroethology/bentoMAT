function entryWindow(gui,ind,data,expt)
...
h.expt.Mouse    = uicontrol(p,ss.C{:},'style','edit','position',ss.box+[10 0 35 0],'string',expt.Mouse);
temp(2)         = uicontrol(p,ss.R{:},'style','text','position',ss.txt+[15 0 50 0],'string','Session');
h.expt.Sess     = uicontrol(p,ss.C{:},'style','edit','position',ss.box+[25 0 35 0],'string',expt.Sess);
temp(3)         = uicontrol(p,ss.R{:},'style','text','position',ss.txt+[35 0 50 0],'string','Trial');
h.expt.Trial    = uicontrol(p,ss.C{:},'style','edit','position',ss.box+[45 0 35 0],'string',expt.Trial);
temp(4)         = uicontrol(p,ss.R{:},'style','text','position',ss.txt+[55 0 50 0],'string','Stim');
h.expt.Stim     = uicontrol(p,ss.C{:},'style','edit','position',ss.box+[350 0 140 0],'string',expt.Stim);
...
uicontrol(h.pTop,ss.C{:},'style','pushbutton','position',[560 5 150 ss.rowsize*2+10],'string','Save Changes',...
          'backgroundcolor',[.65 1 .65],'callback',@addEntryToTable)

guidata(h.f,h);
for f = fieldnames(data)'
    for i=1:length(data.(f{:}))
        dummy.Source.String = {strrep(f{:},'_',' ')};
        dummy.Source.Value = 1;
        entryWindowAddBox(h.f,dummy);
        h=guidata(h.f);
        for d = fieldnames(data.(f{:})(i))'
            h.fields(end).data.(d{:}).String = data.(f{:})(i).(d{:});
        end
    end
end
